function [imOut,xformOut] = registerImagesFast(im,xformIn,fill)
...
if isempty(xformIn)
    ...
    for f = 1:nf                
        if f==1
            ...
            imReg = imtranslate(im(:,:,f),[xformOut(f).affineParams(4), xformOut(f).affineParams(3)],'FillValues',fill(f));
        end
        imOut(:,:,f) = imReg;
    end
else
    ...
    for f = 1:nf       
        if f==1
            ...
            imReg = imtranslate(im(:,:,f),[xform(4), xform(3)],'FillValues',fill(f));
        end
        imOut(:,:,f) = imReg;
