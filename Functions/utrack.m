% function [cellDataTrack, tracksFinal] = utrack(data,trackVar)
tracksFinal = [];
data = cellData;
trackVar = 'Cells';
...
for t = 1:nt
    if strcmp(trackVar,'Nuclei')
        x = data.cNucX(data.Time==t);
        y = data.cNucY(data.Time==t);
    elseif strcmp(trackVar,'Cells')
        x = data.cCellX(data.Time==t);
        y = data.cCellY(data.Time==t);
    end
    dx = 0;
    ...
    points(t).xCoord = [x, dx];
    points(t).yCoord = [y, dy];
    points(t).amp = [data.rCell(data.Time==t), zeros(numel(x),1)];
...
for i = 1:size(tracksFinal,1)
    ...
    for locidx = 1:nTracks
        ...
        for j = 1:length(idxlist)
            ...
            t = tList(j);
            idx = idxlist(j);
            cellDataTrack000 = data(data.Time==t & data.ID==idx,:);
            if j==1
