% function [images, params, im] = loadND2fast(imagelist,imidx,channellist,numFrames,numPositions,params)
% numFrames = [];
clearvars -except imagelist params
channellist = {'DIC','mCherry','GFP'};
numPositions = 1;
numFrames = 2;
imidx = 1;
...
%             images(:,:,frame,position,channel) = imdata;
%             immode = mode(imdata,'all');
...
%             images.([channelname '_mode'])(frame,position) = immode;
        ...
        elseif frame<=numFrames
            images(:,:,frame,position,channel) = imdata;
            immode(frame,position,channel) = mode(imdata,'all');
        end
    end    
end
